...
import {
  DEFAULT_BASE, VIEWPORTS, parseRoutes, routeSlug,
  withBrowser, withPage, settle, waitForAnimations,
...
await withBrowser(browser =>
  Promise.all(routes.flatMap(route => {
    ...
    return Object.entries(VIEWPORTS).map(([name, viewport]) =>
      withPage(browser, viewport, async page => {
        ...
        await page.screenshot({ path: `${OUT}/${slug}-${name}-fold.png`, fullPage: false });
        ...
        if (name === 'desktop') {
          ...
          for (const section of sections) {
            ...
            await section.screenshot({ path: `${OUT}/${slug}-section-${id}.png` });
          ...
          if (isFirstRoute) {
            ...
            await page.screenshot({ path: `${OUT}/desktop-scrolled-nav.png`, fullPage: false });

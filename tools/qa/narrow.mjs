...
import { DEFAULT_BASE, parseRoutes, routeSlug, withBrowser, withPage, settle } from './lib/browser.mjs';

const BASE   = process.argv[2] || DEFAULT_BASE;
const routes = parseRoutes(process.argv[3]);
const OUT    = process.argv[4] || '.tmp/shots';
const widths = (process.argv[5] || '320,360').split(',').map(Number).filter(Boolean);
mkdirSync(OUT, { recursive: true });

const checks = routes.flatMap(route => widths.map(w => ({ route, w })));

const results = await withBrowser(browser =>
  Promise.all(checks.map(({ route, w }) =>
    withPage(browser, { width: w, height: 760 }, async page => {
      ...
      await page.screenshot({ path: `${OUT}/narrow-${routeSlug(route)}-${w}.png`, fullPage: true });
...
for (const result of results) {
  ...
  console.log(`${status}  ${result.route}  w=${result.w}  docW=${result.docW}  winW=${result.winW}`);
